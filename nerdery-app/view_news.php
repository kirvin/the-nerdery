...
require_once ('includes/class.dropshadow.php');
	...
	if ($page_action == "insertNewsItem") {
		...
		$upload_path = getJournalItemsUploadDir() . "/" . $journal_id;
		$file_name = $nid;
...
//			$td = getTempDir();
//			$ud = getJournalItemsUploadDir() . "/" . $journal_id . "/";
...
//		// finish file upload
//		if (strlen ($_FILES["item_file"]["name"]) > 0) {
//			$result = move_uploaded_file ($_FILES["item_file"]["tmp_name"], $td . $_FILES["item_file"]["name"]);
//			var_dump ($result);
//			if ($result) {
//				$new_file = $ud . $nid;
//				if (!file_exists ($ud))
